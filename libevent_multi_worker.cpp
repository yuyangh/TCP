...
void ListenAccept(int sock, short event, void *arg) {
	...
	int new_client_fd = accept(sock, (struct sockaddr *) &client_addr, &length);
...
int main(int argc, char **argv) {
	...
	for (int i = 0; i < NUM_WORKERS; i++) {
		...
		LibeventThreads[i].thread_event = event_new(LibeventThreads[i].base, LibeventThreads[i].read_fd,
	...
	ListenEvent = event_new(DispatcherThread.base, server_fd, EV_READ | EV_PERSIST, ListenAccept, nullptr);
